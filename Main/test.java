...
public class test {
    ...
    private static void purchaseTicket() {
        List<Activity> activities = ticketOperation.getAvailableActivities();
        if (activities.isEmpty()) {
            System.out.println("No activities available for ticket purchase.");
            return;
        }
        ...
        if (ticketOperation.purchaseTicket(currentUser, selectedActivity, quantity)) {
            System.out.println("Ticket purchase successful!");
        } else {
            System.out.println("Ticket purchase failed, possibly insufficient tickets.");
        }
    }
